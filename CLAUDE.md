...
- **Export .ics** (`services/ics.py`, `GET /trips/{id}/calendar.ics?bookings=`): VEVENTs generados a mano (escape+folding 75 octetos+CRLF, UIDs estables, DTEND exclusivo en all-day, horas flotantes sin TZID). Reservas transporte → "Vuelo: MAD → Tokio"; las enlazadas a items no se duplican. **Suscripción**: `Trip.ics_token` (POST `/trips/{id}/ics-token` genera/rota) alimenta el feed público `GET /api/v1/calendar/{token}.ics` para Google Calendar «Desde URL» (diálogo "Suscribirse" en Itinerario); la ruta debe quedar exenta de auth en el reverse proxy y el SW no la cachea.
