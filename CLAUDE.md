...
- Navegación principal: Viajes (/), Mapa (/mapa), Maletas (/maletas), Viajeros (/viajeros), Ajustes (/settings, solo categorías).
...
- Geocoding con `accept-language=es` (Nominatim devuelve "Taipéi", no 臺北). Tiles de CARTO sin API key (voyager en claro, dark_all en oscuro) vía `useMapTiles`; el backend pina Python 3.12 con `.python-version` (uv) para sobrevivir a los upgrades de Python del sistema.
